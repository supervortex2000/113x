...
# Add Wake on Wlan for WiFi Wake host
if [ -f $1/etc/wpa_supplicant.conf ] ; then
	echo "wowlan_triggers=any" >>  $1/etc/wpa_supplicant.conf
fi


# Change BT name to amlogic-sbr from amlogic
...
# Change gst soundcard  name to amlogic-sbr for avs audio source
if [ -f $1/etc/gst-soundcard.conf ]; then
	sed -i 's/hw=\"dmixer_avs_auto\"/hw=\"avs_output\"/g' $1/etc/gst-soundcard.conf
fi

# Switch default_audioservice.conf
# rm -frv $1/etc/default_audioservice.conf
# mv $1/etc/default_audioservice_datmos_V1_7.conf $1/etc/default_audioservice.conf
date > $1/etc/image_timestamp
...
# rm -frv $1/etc/init.d/S39wifi

# rm -frv $1/etc/init.d/S40network
# rm -frv $1/etc/init.d/S41inetd
#rm -frv $1/etc/init.d/S48avs
#rm -frv $1/etc/init.d/avs_mdns.sh
#rm -frv $1/etc/init.d/avs_moniter.sh
