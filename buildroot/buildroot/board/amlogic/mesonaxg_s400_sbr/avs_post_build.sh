...
if [ -f $1/etc/init.d/S82airplay2 ] ; then
	res=$(echo $1 | grep "k54")
	if [ "$res" != "" ]; then
		sed -i 's/OPTIONS=.*/OPTIONS=\"-D dmixer_avs_auto --ipc-client \/tmp\/homeapp_airplay --mfi-address 0x10 --mfi-device \/dev\/i2c-0\"/' $1/etc/init.d/S82airplay2
	else
...
# mv $1/etc/default_audioservice.conf_avs $1/etc/default_audioservice.conf
