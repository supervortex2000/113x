...
# AF401 no need S44bluetooth
if [ -f $TARGET_DIR/etc/init.d/S44bluetooth ]; then
    rm -fr $TARGET_DIR/etc/init.d/S44bluetooth
fi
...
echo "#This is auto generated build information" > $TARGET_DIR/etc/buildinfo
echo "BuildTime: $(date)" >> $TARGET_DIR/etc/buildinfo
echo "BuildPlatform: ${USER}@$(hostname)" >> $TARGET_DIR/etc/buildinfo
if [ -n "$JOB_NAME" ]; then
   echo "JenkinsJob: $BUILD_NUMBER@$JOB_NAME" >> $TARGET_DIR/etc/buildinfo
fi

if [ -d $BR_PATH/vendor/amlogic/ipc/ipc-sdk/.git ]; then
  echo "IPC-SDK REV: $(git -C $BR_PATH/vendor/amlogic/ipc/ipc-sdk/ describe --abbrev=8 --dirty --always --tags)" >> $TARGET_DIR/etc/buildinfo
fi

if [ -d $BR_PATH/vendor/amlogic/ipc/ipc-refapp/.git ]; then
  echo "IPC-REFAPP REV: $(git -C $BR_PATH/vendor/amlogic/ipc/ipc-refapp describe --abbrev=8 --dirty --always --tags)" >> $TARGET_DIR/etc/buildinfo
fi

if [ -d $BR_PATH/vendor/amlogic/ipc/ipc_webui/.git ]; then
  echo "IPC-WEBUI REV: $(git -C $BR_PATH/vendor/amlogic/ipc/ipc_webui describe --abbrev=8 --dirty --always --tags)" >> $TARGET_DIR/etc/buildinfo
fi
