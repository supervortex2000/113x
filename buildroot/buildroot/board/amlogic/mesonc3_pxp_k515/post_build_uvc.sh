...
#ADB should be disabled, as use it as UVC
if [ -f $TARGET_DIR/etc/init.d/S89usbgadget ]; then
    mv $TARGET_DIR/etc/init.d/S89usbgadget $TARGET_DIR/etc/
fi
...
if [ -d $BR_PATH/vendor/amlogic/uvc-gadget/.git ]; then
  echo "UVC-GADGET REV: $(git -C $BR_PATH/vendor/amlogic/uvc-gadget describe --abbrev=8 --dirty --always --tags)" >> $TARGET_DIR/etc/buildinfo
fi
