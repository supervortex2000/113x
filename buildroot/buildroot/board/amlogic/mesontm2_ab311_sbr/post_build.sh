...
if [ -f $1/etc/init.d/S82airplay2 ] ; then
	...
	sed -i 's/OPTIONS=.*/OPTIONS=\"-D dmixer_avs_auto --ipc-client \/tmp\/homeapp_airplay --mfi-device \/dev\/i2c-0 --mfi-address 0x10 --enable-res-management 1\"/' $1/etc/init.d/S82airplay2
fi
