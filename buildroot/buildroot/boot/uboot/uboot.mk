...
ifeq ($(BR2_TARGET_UBOOT_AMLOGIC_REPO),y)
	...
	UBOOT_BINS_NEW := build/u-boot.bin.signed build/u-boot.bin.sd.bin.signed build/u-boot.bin.usb.signed
	C3_NAND_BINS := fip/_tmp/bb1st.sto.bin.signed fip/_tmp/blob-bl2e.sto.bin.signed fip/_tmp/blob-bl2x.bin.signed fip/_tmp/device-fip.bin.signed
endif

ifeq ($(BR2_TARGET_UBOOT_AMLOGIC_M8B),y)
	UBOOT_BINS := build/u-boot.bin build/u-boot-usb.bin build/u-boot-comp.bin build/ddr_init.bin
endif
...
else ifeq ($(BR2_TARGET_UBOOT_PLATFORM), "t5")
SECUROS_IMAGE_DIR = "t5"
else ifeq ($(BR2_TARGET_UBOOT_PLATFORM), "t5d")
SECUROS_IMAGE_DIR = "t5d"
else
SECUROS_IMAGE_DIR = "gx"
endif
...
ifneq ($(BR2_PACKAGE_TDK_VERSION),"3.8")
...
define UBOOT_COPY_GIT_DIR
		test -d $(UBOOT_SITE)/.git && ln -sf $(UBOOT_SITE)/.git $(@D)/ || true
endef
...
UBOOT_DEPENDENCIES += aml-uboot-customer
...
define UBOOT_APPLY_AML_PATCHES
	test -d $(BR2_PACKAGE_AML_UBOOT_PATCHDIR) && $(APPLY_PATCHES) $(@D) $(BR2_PACKAGE_AML_UBOOT_PATCHDIR) \* || true
endef
UBOOT_POST_PATCH_HOOKS = UBOOT_APPLY_AML_PATCHES
...
ifeq ($(BR2_TARGET_UBOOT_AMLOGIC_REPO),y)
ifeq ($(strip $(BR2_PACKAGE_AML_ARM_RTOS_SDK_BUILD_OPTION)),)
define UBOOT_BUILD_CMDS
$(TARGET_CONFIGURE_OPTS) $(UBOOT_CONFIGURE_OPTS); \
	cd $(@D); \
	REPO_DIR=$(BR2_PACKAGE_AOCPU_RTOS_SDK_LOCAL_PATH) \
	CROSS_COMPILE=$(BR2_TARGET_UBOOT_CUSTOM_COMPILE64) \
	CROSS_COMPILE_T32=$(BR2_TARGET_UBOOT_CUSTOM_COMPILE32) \
	./mk $(UBOOT_BOARD_NAME) $(BR2_PACKAGE_UBOOT_EXTRA_ARGS) ;
endef
endif
#define UBOOT_INSTALL_AMLOGIC_USB_TOOL
#	@if test -f  $(@D)/build/u-boot.bin; then 						\
#	echo "find regular uboot";									\
...
#	fi
#	@if test -f $(@D)/build/u-boot.bin.signed; then 				\
#	echo "find signed uboot";										\
#	cp -dpf $(@D)/build/u-boot.bin.signed $(BINARIES_DIR)/;			\
#	cp -dpf $(@D)/build/u-boot.bin.sd.bin.signed $(BINARIES_DIR)/;	\
#	fi
#endef
define UBOOT_INSTALL_AMLOGIC_USB_TOOL
    $(INSTALL) -m 0755 $(@D)/fip/$(call qstrip,$(BR2_TARGET_UBOOT_PLATFORM))/aml_encrypt_$(BR2_TARGET_UBOOT_PLATFORM) $(HOST_DIR)/usr/bin
...
ifeq ($(BR2_TARGET_UBOOT_AMLOGIC_M8B),y)
define UBOOT_INSTALL_AMLOGIC_USB_TOOL
	cp -dpf $(@D)/board/amlogic/$(BR2_TARGET_UBOOT_BOARDNAME)/aml-aes-key.aes $(BINARIES_DIR)/; \
	cp -dpf $(@D)/board/amlogic/$(BR2_TARGET_UBOOT_BOARDNAME)/aml-rsa-key.k1a $(BINARIES_DIR)/; \
	cp -dpf $(@D)/board/amlogic/$(BR2_TARGET_UBOOT_BOARDNAME)/aml-rsa-key.k2a $(BINARIES_DIR)/; \
	...
	$(INSTALL) -m 0755 $(@D)/tools/secu_boot/aml_encrypt_m8b $(HOST_DIR)/usr/bin
endef
UBOOT_POST_INSTALL_IMAGES_HOOKS += UBOOT_INSTALL_AMLOGIC_USB_TOOL
define UBOOT_BUILD_CMDS
	cd $(@D); make $(UBOOT_BOARD_NAME)_config; make
endef
else
...
define UBOOT_INSTALL_IMAGES_CMDS
	#This is for traditional case
	$(if -f $(@D)/build/u-boot.bin,
	$(foreach f,$(UBOOT_BINS), \
		test -f $(@D)/$(f) && cp -dpf $(@D)/$(f) $(BINARIES_DIR)/ || echo "Potential error: Missing $(@D)/$(f)"
	))
	#This is for SC2 case
	$(if -f $(@D)/build/u-boot.bin.signed,
	$(foreach f,$(UBOOT_BINS_NEW), \
		test -f $(@D)/$(f) && cp -dpf $(@D)/$(f) $(BINARIES_DIR)/ || echo "Potential error: Missing $(@D)/$(f)"
	))
	#This is for C3 nand case
	$(foreach f,$(C3_NAND_BINS), \
		test -f $(@D)/$(f) && cp -dpf $(@D)/$(f) $(BINARIES_DIR)/ || echo "Potential error: Missing $(@D)/$(f)"
	)
	$(if $(BR2_TARGET_UBOOT_FORMAT_NAND),
