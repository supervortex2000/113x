...
#eval set -- "$args"
#while [ $# -gt 0 ]; do
while getopts "w:p:f:u:dh" opt; do
    case $opt in
        w)
...
AML_PATCHES_FILE_TMP=$(mktemp /tmp/aml_br_patch_file_XXXXXX.tgz )
AML_PATCHES_PATH_TMP=$(mktemp -d /tmp/aml_br_patch_folder_XXXXXX)

function check_input_param() {
    local _RET_VALUE=0

    if [ -f $SOURCE_PATH/.aml-br-patch-done ]; then
        echo "Patches already be applied to this workspace: $SOURCE_PATH"
        echo "Please clean up and delete .aml-br-patch-done in workspace"
        _RET_VALUE=1
    fi
...
if check_input_param; then
  touch $SOURCE_PATH/.aml-br-patch-done
  ...
  do
        ...
        do
            ...
            if git -C $PROJ_PATH apply --ignore-whitespace --check $AML_PATCHES_PATH/$PatchFolder/$Patch; then
                git -C $PROJ_PATH am --ignore-whitespace $AML_PATCHES_PATH/$PatchFolder/$Patch
            else
