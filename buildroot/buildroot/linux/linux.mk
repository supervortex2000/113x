...
LINUX_KERNEL_COMMON_DRIVERS = $(call qstrip,$(BR2_LINUX_KERNEL_COMMON_DRIVERS))
...
ifneq ($(LINUX_KERNEL_COMMON_DRIVERS),)
export COMMON_DRIVERS_DIR=${LINUX_KERNEL_COMMON_DRIVERS}
export ROOT_DIR=$(LINUX_DIR)/../
export KERNEL_DIR=linux-$(LINUX_VERSION)
export MODULES_STAGING_DIR=$(TARGET_DIR)
export DIST_DIR=$(TARGET_DIR)/kernel_dist
export OUT_AMLOGIC_DIR=$(TARGET_DIR)/amlogic
LINUX_MAKE_ENV += \
    COMMON_DRIVERS_DIR=${LINUX_KERNEL_COMMON_DRIVERS} \
    $(NULL)
endif
...
ifeq ($(BR2_PACKAGE_AML_TVIN),y)
	LINUX_DEPENDENCIES += aml-tvin
endif
ifeq ($(BR2_PACKAGE_AML_PMU),y)
	LINUX_DEPENDENCIES += aml-pmu
endif
ifeq ($(BR2_PACKAGE_AML_TOUCH),y)
	LINUX_DEPENDENCIES += aml-touch
endif

SYSROOT_ARCH=$(KERNEL_ARCH)
ifeq ($(BR2_arm),y)
	SYSROOT_ARCH=$(ARCH)
endif
...
LINUX_ARCH_DTB_PATH = $(LINUX_DIR)/${LINUX_KERNEL_COMMON_DRIVERS}/arch/$(KERNEL_ARCH)
endif
...
BR2_PACKAGE_AML_LINUX_PATCHDIR=$(BR2_PACKAGE_AML_KERNEL_PATCHDIR)
define LINUX_APPLY_AML_PATCHES
	test -d $(BR2_PACKAGE_AML_LINUX_PATCHDIR) && $(APPLY_PATCHES) $(@D) $(BR2_PACKAGE_AML_LINUX_PATCHDIR) \* || true
endef
LINUX_POST_PATCH_HOOKS += LINUX_APPLY_AML_PATCHES
LINUX_POST_PATCH_HOOKS += LINUX_APPLY_GKI_PATCHES

ifneq ($(LINUX_KERNEL_COMMON_DRIVERS),)
define LINUX_COPY_AML_DEFCONFIG
	if test -f $(@D)/${LINUX_KERNEL_COMMON_DRIVERS}/arch/$(KERNEL_ARCH)/configs/${LINUX_KCONFIG_DEFCONFIG}; then \
		cp -f $(@D)/${LINUX_KERNEL_COMMON_DRIVERS}/arch/$(KERNEL_ARCH)/configs/${LINUX_KCONFIG_DEFCONFIG} $(@D)/arch/$(KERNEL_ARCH)/configs/; \
	fi
endef
LINUX_POST_PATCH_HOOKS += LINUX_COPY_AML_DEFCONFIG
endif
...
define LINUX_KCONFIG_FIXUP_CMDS
	...
	$(call KCONFIG_ENABLE_OPT,CONFIG_AMLOGIC_SND_SPLIT_MODE_MMAP,$(@D)/.config)
	...
	$(if $(BR2_PACKAGE_MTD),
		$(call KCONFIG_ENABLE_OPT,CONFIG_AMLOGIC_NAND,$(@D)/.config)
		$(call KCONFIG_DISABLE_OPT,CONFIG_PWRSEQ_EMMC,$(@D)/.config))
	...
	$(if $(BR2_TARGET_ROOTFS_EXT2),
		$(call KCONFIG_ENABLE_OPT,CONFIG_EXT4_FS,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_EXT4_FS_POSIX_ACL,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_EXT4_FS_SECURITY,$(@D)/.config))

	$(if $(BR2_PACKAGE_AML_ARM_RTOS_SDK),
		$(call KCONFIG_ENABLE_OPT,CONFIG_SQUASHFS,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_SQUASHFS_XATTR,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_SQUASHFS_ZSTD,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_AMLOGIC_FREERTOS_C3,$(@D)/.config)
		$(call KCONFIG_DISABLE_OPT,CONFIG_SCSI,$(@D)/.config)
		$(call KCONFIG_DISABLE_OPT,CONFIG_VT,$(@D)/.config)
		$(call KCONFIG_DISABLE_OPT,CONFIG_UNIX98_PTYS,$(@D)/.config)
		$(call KCONFIG_DISABLE_OPT,CONFIG_MODULE_UNLOAD,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_AMLOGIC_PINCTRL_MESON_C3,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_AMLOGIC_I2C_MESON,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_AMLOGIC_GPIO,$(@D)/.config))
		$(call KCONFIG_ENABLE_OPT,CONFIG_AMLOGIC_MEMORY_DEBUG,$(@D)/.config)

	$(if $(BR2_PACKAGE_IPC_REFERENCE),
		$(call KCONFIG_SET_OPT,CONFIG_VFAT_FS,m,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_FAT_DEFAULT_UTF8,m,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_EXFAT_FS,m,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_AMLOGIC_BATTERY_CW2015,m,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_AMLOGIC_RTC_IT8563,m,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_IIO_BUFFER,y,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_IIO,m,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_AMLOGIC_MESON_SARADC,m,$(@D)/.config)
		$(call KCONFIG_SET_OPT,CONFIG_AMLOGIC_IIO_KFIFO_BUF,m,$(@D)/.config))

	$(if $(filter "mesonc3", $(BR2_TARGET_BOARD_PLATFORM)),
...
define LINUX_STRIP_DTD
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) $(LINUX_DTDS)
endef
...
define LINUX_INSTALL_DTB
	...
	cp $(addprefix \
		$(LINUX_ARCH_DTB_PATH)/boot/$(if $(wildcard $(addprefix \
		$(LINUX_ARCH_DTB_PATH)/boot/dts/amlogic/,$(AML_DTBS))),dts/amlogic/),$(AML_DTBS)) \
		$(BINARIES_DIR)/
endef
...
#GKI configurtations
ifeq ($(BR2_LINUX_KERNEL_GKI),y)
define LINUX_BUILD_GKI
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) GKI_EXT_MODULE_PREDEFINE="$$(cat $(STAGING_DIR)/gki_ext_module_predefine)" GKI_EXT_MODULE_CONFIG="$$(cat $(STAGING_DIR)/gki_ext_module_config)" -C $(@D)/$(1) M=$(1) KERNEL_SRC=$(@D)
endef

define LINUX_INSTALL_GKI
	pushd $(@D); rsync -R $$(find $(1) -name *.ko | xargs) $(TARGET_DIR)/lib/modules/$(LINUX_VERSION_PROBED)/kernel/; popd
endef

define LINUX_APPLY_GKI_PATCHES
	pushd $(LINUX_PKGDIR); cp -af gki-read_ext_module_predefine.sh gki-read_ext_module_config.sh $(@D)/scripts/amlogic/; popd;
endef
endif
...
define LINUX_BUILD_CMDS
	...
	$(LINUX_BUILD_DTB)
	$(LINUX_APPEND_DTB)
	$(if $(BR2_LINUX_KERNEL_GKI),
		($(@D)/scripts/amlogic/gki-read_ext_module_predefine.sh $(@D)/arch/$(KERNEL_ARCH)/configs/$(BR2_LINUX_KERNEL_GKI_CONFIG) > $(STAGING_DIR)/gki_ext_module_predefine)
		($(@D)/scripts/amlogic/gki-read_ext_module_config.sh $(@D)/arch/$(KERNEL_ARCH)/configs/$(BR2_LINUX_KERNEL_GKI_CONFIG) > $(STAGING_DIR)/gki_ext_module_config)
		$(call LINUX_BUILD_GKI,drivers/amlogic)
		$(call LINUX_BUILD_GKI,sound/soc/amlogic)
		$(call LINUX_BUILD_GKI,sound/soc/codecs/amlogic))
	$(LINUX_MAKE_ENV) $(MAKE) ARCH=$(SYSROOT_ARCH) -C $(@D) headers_install INSTALL_HDR_PATH=$(STAGING_DIR)/usr
endef
...
endef
define LINUX_INSTALL_VMLINUX
	$(INSTALL) -m 0644 -D $(LINUX_DIR)/vmlinux $(1)
endef

define LINUX_INSTALL_IMAGES_CMDS
	$(call LINUX_INSTALL_IMAGE,$(BINARIES_DIR))
	$(call LINUX_INSTALL_VMLINUX,$(BINARIES_DIR))
...
define LINUX_INSTALL_TARGET_CMDS
	...
	@if test -f ${ROOT_DIR}/${KERNEL_DIR}/${COMMON_DRIVERS_DIR}/amlogic_utils.sh; then \
		if test -f $(BR2_LINUX_KERNEL_MODULES_SEQUENCE_LIST); then \
			cp $(BR2_LINUX_KERNEL_MODULES_SEQUENCE_LIST) ${ROOT_DIR}/${KERNEL_DIR}/${COMMON_DRIVERS_DIR}/scripts/amlogic/modules_sequence_list -f; \
		fi; \
		source ${ROOT_DIR}/${KERNEL_DIR}/${COMMON_DRIVERS_DIR}/amlogic_utils.sh; OUT_DIR=$(KERNEL_DIR) modules_install $(BR2_LINUX_KERNEL_MODULE_SOC_FAMILY); \
	fi
	$(if $(BR2_LINUX_KERNEL_GKI),
		$(call LINUX_INSTALL_GKI,drivers/amlogic)
		$(call LINUX_INSTALL_GKI,sound/soc/amlogic)
		$(call LINUX_INSTALL_GKI,sound/soc/codecs/amlogic)
...
define LINUX_RM_MODULES_ALIAS_BIN
	rm -f $(TARGET_DIR)/lib/modules/$(LINUX_VERSION_PROBED)/modules.alias.bin
endef

#Blance 2020-4-21, run additional depmod will cause wifi driver auto load, which is not expected
LINUX_TARGET_FINALIZE_HOOKS += LINUX_RUN_DEPMOD LINUX_RM_MODULES_ALIAS_BIN
