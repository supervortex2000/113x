...
AML_RTOS_DEPENDENCIES += aml-dsp-util
...
define AML_RTOS_BUILD_CMDS
	...
	if [ -n "$(BR2_PACKAGE_AML_RTOS_DSPA_BUILD_OPTION)" ]; then \
		pushd $(@D);  \
			...
			test -f ./out_dsp/dspboot.bin &&  $(INSTALL) -D -m 644 ./out_dsp/dspboot.bin $(BINARIES_DIR)/dspbootA.bin || true;\
			test -f ./out_dsp/dspboot_sram.bin &&  $(INSTALL) -D -m 644 ./out_dsp/dspboot_sram.bin $(BINARIES_DIR)/dspbootA_sram.bin || true;\
			test -f ./out_dsp/dspboot_sdram.bin &&  $(INSTALL) -D -m 644 ./out_dsp/dspboot_sdram.bin $(BINARIES_DIR)/dspbootA_sdram.bin || true;\
		popd; \
	fi
	if [ -n "$(BR2_PACKAGE_AML_RTOS_DSPB_BUILD_OPTION)" ]; then \
		pushd $(@D);  \
			set -e; ./scripts/amlogic/mk.sh $(BR2_PACKAGE_AML_RTOS_DSPB_BUILD_OPTION); \
			test -f ./out_dsp/dspboot.bin &&  $(INSTALL) -D -m 644 ./out_dsp/dspboot.bin $(BINARIES_DIR)/dspbootB.bin || true;\
			test -f ./out_dsp/dspboot_sram.bin &&  $(INSTALL) -D -m 644 ./out_dsp/dspboot_sram.bin $(BINARIES_DIR)/dspbootB_sram.bin || true;\
			test -f ./out_dsp/dspboot_sdram.bin &&  $(INSTALL) -D -m 644 ./out_dsp/dspboot_sdram.bin $(BINARIES_DIR)/dspbootB_sdram.bin || true;\
		popd; \
	fi
endef
