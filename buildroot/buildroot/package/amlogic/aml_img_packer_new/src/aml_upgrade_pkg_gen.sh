...
then
	...
	if [ "${absystem}" != "absystem" ]; then
	        aml_upgrade_package_conf=${PRODUCT_AML_IMG_PACK_DIR}/aml_upgrade_package_emmc_squashfs.conf
	else
		    aml_upgrade_package_conf=${PRODUCT_AML_IMG_PACK_DIR}/aml_upgrade_package_emmc_squashfs_ab.conf
	fi
else
	if [ "${absystem}" != "absystem" ]; then
