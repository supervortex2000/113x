...
ifneq ($(BR2_PACKAGE_AML_BRCM_BSA),y)
define BLUEZ5_UTILS_INSTALL_AML_TOOLS
	...
	$(INSTALL) -D -m 0755 package/amlogic/bluez5-utils/bluez_tool.sh $(TARGET_DIR)/usr/bin
	$(INSTALL) -D -m 0644 package/amlogic/bluez5-utils/main.conf $(TARGET_DIR)/etc/bluetooth
	@echo "Bluez setting device: $(Device)"
	sed -i "/debug/aDevice=$(Device)" $(TARGET_DIR)/etc/bluetooth/main.conf
	cp -f $(TARGET_DIR)/etc/bluetooth/main.conf $(TARGET_DIR)/etc/bluetooth/main_bkp.conf
	cp -f $(TARGET_DIR)/usr/bin/bluez_tool.sh $(TARGET_DIR)/etc/init.d/S44bluetooth
endef
endif
