...
function get_alsa_device_from_conf()
{
	if [ -f $alsa_file ];then
		str=`grep -Insr "spk=" $alsa_file`
		if [ ! $str = "" ];then
			spk=`echo $str | awk -F = '{print $2}'`
		else
			echo No alsa device defined in confirue file
		fi
	else
		echo "No alsa_bt.conf"
	fi
}
...
function set_btname()
{
	name_file=/sys/class/net/wlan0/address
	local cnt=1
	bt_name="chatGPT"
	while [ $cnt -lt 10 ]
	do
		if [ -f $name_file ];then
			bt_name=`cat $name_file | sed s/://g`
			if [ "$bt_name" != "" ];then
				break
			fi
		else
...
brcm_bt_init()
{
	brcm_patchram_plus --enable_hci --baudrate 2000000 --use_baudrate_for_download --patchram /etc/bluetooth/BCM4354.hcd /dev/ttyS1 --no2bytes &
}
A2DP_SINK_SERVICE()
{
