...
# gpu version num: r37p0 -> 370
GPU_VER_NUM=`echo $(GPU_VERSION) | tr -cd "[0-9]"`

UTGARD_BUILD_CMD = \
	if [ -e $(@D)/utgard/Makefile ]; then \
		cd $(@D)/utgard ;\
		$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(@D)/utgard M=../gpu-$(GPU_VERSION)/utgard KERNEL_SRC=$(LINUX_DIR) \
...
BIFROST_BUILD_CMD = \
	if [ -e $(@D)/bifrost/$(GPU_VERSION) ]; then \
		...
		echo "dvalin build" ; \
		if [ $(GPU_VER_NUM) -ge 370 ]; then \
			$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(LINUX_DIR) M=`pwd` \
			ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(TARGET_KERNEL_CROSS) \
			EXTRA_CFLAGS="-DCONFIG_MALI_MIDGARD_DVFS -DCONFIG_MALI_LOW_MEM=$(GPU_LOW_MEM)" \
			EXTRA_CFLAGS+="-I${@D}/bifrost/$(GPU_VERSION)/kernel/include" \
			CONFIG_MALI_MIDGARD=m CONFIG_MALI_MIDGARD_DVFS=y CONFIG_MALI_PLATFORM_NAME="devicetree" modules; \
		else \
			$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(LINUX_DIR) M=`pwd` \
			ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(TARGET_KERNEL_CROSS) \
			EXTRA_CFLAGS="-DCONFIG_MALI_PLATFORM_DEVICETREE -DCONFIG_MALI_MIDGARD_DVFS -DCONFIG_MALI_BACKEND=gpu" \
			EXTRA_CFLAGS+="-DCONFIG_MALI_LOW_MEM=$(GPU_LOW_MEM) -I${@D}/bifrost/$(GPU_VERSION)/kernel/include" \
