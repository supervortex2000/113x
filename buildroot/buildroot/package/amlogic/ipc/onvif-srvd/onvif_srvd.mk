...
ONVIF_SRVD_DEPENDENCIES += ipc-property
...
ONVIF_SRVD_GIT_VERSION=$(shell \
			if [ -d $(ONVIF_SRVD_INTERNAL_SITE)/.git ]; then \
			   git -C $(ONVIF_SRVD_INTERNAL_SITE) describe --abbrev=8 --dirty --always --tags; \
			else \
			   echo "unknown"; \
			fi)

define ONVIF_SRVD_BUILD_CMDS
	REVISION=$(ONVIF_SRVD_GIT_VERSION) $(TARGET_MAKE_ENV) \
