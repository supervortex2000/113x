...
ONVIF_WSDD_DEPENDENCIES += ipc-property
...
ONVIF_WSDD_GIT_VERSION=$(shell \
			if [ -d $(ONVIF_WSDD_INTERNAL_SITE)/.git ]; then \
			   git -C $(ONVIF_WSDD_INTERNAL_SITE) describe --abbrev=8 --dirty --always --tags; \
			else \
			   echo "unknown"; \
			fi)
...
define ONVIF_WSDD_BUILD_CMDS
	REVISION=$(ONVIF_WSDD_GIT_VERSION) $(TARGET_MAKE_ENV) \
