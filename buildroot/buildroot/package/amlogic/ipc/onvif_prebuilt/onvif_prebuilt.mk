...
ONVIF_PREBUILT_DEPENDENCIES = onvif-srvd
ONVIF_PREBUILT_DEPENDENCIES += onvif-wsdd
ONVIF_PREBUILT_DEPENDENCIES += ipc-webui

ifeq ($(ONVIF_PREBUILT_LEGACY),y)
ONVIF_PREBUILT_DEPENDENCIES += onvif-rtsp
ifeq ($(BR2_PACKAGE_AML_SOC_FAMILY_NAME), "C1")
ONVIF_PREBUILT_DEPENDENCIES += ipc-autocap
endif
...
define ONVIF_PREBUILT_INSTALL_TARGET_CMDS
	...
	cp -a $(TARGET_DIR)/etc/ipc                            $(@D)/$(ONVIF_SUB_PATH)/etc/
	cp -a $(TARGET_DIR)/etc/init.d/S45ipc-property         $(@D)/$(ONVIF_SUB_PATH)/etc/init.d/
	cp -a $(TARGET_DIR)/etc/init.d/S45ipc-system-service   $(@D)/$(ONVIF_SUB_PATH)/etc/init.d/
	...
	cp -a $(TARGET_DIR)/usr/bin/ipc-system-service         $(@D)/$(ONVIF_SUB_PATH)/usr/bin/
	...
	cp -a $(TARGET_DIR)/usr/lib/libgstamlionallocator-1.0.so*        $(@D)/$(ONVIF_SUB_PATH)/usr/lib/
	...
	cp -a $(TARGET_DIR)/etc/php-fpm.conf           $(@D)/$(ONVIF_SUB_PATH)/etc/php-fpm.conf
