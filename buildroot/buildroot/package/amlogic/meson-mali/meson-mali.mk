...
else ifeq ($(BR2_PACKAGE_MESON_MALI_GBM_EGL),y)
EGL_PLATFORM_HEADER = EGL_platform/platform_gbm
MALI_LIB_LOC = $(MALI_VERSION)/$(API_VERSION)/gbm
else
...
MALI_LIB_DIR = eabi/$(MALI_LIB_LOC)
endif
...
define WAYLAND_DRM_INSTALL_STAGING
	$(LIBGBM_INSTALL_STAGING)
endef
endif

ifeq ($(BR2_PACKAGE_MESON_MALI_GBM_EGL),y)
define GBM_INSTALL_STAGING
	$(LIBGBM_INSTALL_STAGING)
endef
endif
...
define BASE_INSTALL_TARGET
	...
	if [ -e $(MESON_MALI_DIR)/lib/$(MALI_LIB_DIR)/../../mali.json ] ; then \
		mkdir -p $(TARGET_DIR)/usr/share/vulkan/icd.d; \
		install -m 644 $(MESON_MALI_DIR)/lib/$(MALI_LIB_DIR)/../../mali.json $(TARGET_DIR)/usr/share/vulkan/icd.d/; \
	fi
endef

define LIBGBM_INSTALL_TARGET
...
define WAYLAND_DRM_INSTALL_TARGET
	$(LIBGBM_INSTALL_TARGET)
	echo wayland_drm > $(MESON_EGL_CONF)
endef
endif

ifeq ($(BR2_PACKAGE_MESON_MALI_GBM_EGL),y)
define GBM_INSTALL_TARGET
	$(LIBGBM_INSTALL_TARGET)
	echo gbm > $(MESON_EGL_CONF)
endef
endif
...
ifeq ($(BR2_PACKAGE_MESON_MALI_RDK),y)
define MESON_MALI_RDK_INSTALL_TARGET
	if [ -f $(TARGET_DIR)/usr/lib/libMali_rdk.so ] ; then \
		rm -f $(TARGET_DIR)/usr/lib/libMali.so; \
		mv $(TARGET_DIR)/usr/lib/libMali_rdk.so $(TARGET_DIR)/usr/lib/libMali.so; \
	else \
		echo "Error! These is no libMali_rdk.so exist"; \
		exit 1; \
	fi
endef
endif
