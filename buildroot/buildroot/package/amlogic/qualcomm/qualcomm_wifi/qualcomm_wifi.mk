...
define QUALCOMM_QCA6174_BUILD_CMDS
	$(TARGET_CONFIGURE_OPTS) $(MAKE)  -C $(@D)/qca6174/AIO/build ARCH=$(KERNEL_ARCH) \
		KERNELPATH=$(LINUX_DIR)	CROSS_COMPILE=$(TARGET_KERNEL_CROSS)  CONFIG_BUILDROOT=y
endef
ifeq ($(BR2_LINUX_KERNEL_CUSTOM_LOCAL_VERSION_VALUE),"amlogic-5.4-dev")
define QUALCOMM_QCA6174_BUILD_CMDS
	$(TARGET_CONFIGURE_OPTS) $(MAKE)  -C $(@D)/qca6174/AIO/build M=$(@D)/qca6174/AIO/build ARCH=$(KERNEL_ARCH)  \
		KERNEL_SRC=$(LINUX_DIR) KERNELPATH=$(LINUX_DIR)	CROSS_COMPILE=$(TARGET_KERNEL_CROSS)  CONFIG_BUILDROOT=y
endef
endif
