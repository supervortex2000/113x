...
LIB_SIMPLE_FRAMEWORK3 = flatland/64bit/libsimple_framework3.so
LIB_PATH=$(STAGING_DIR)/usr/lib64
else ifeq ($(BR2_ARM_EABIHF),y)
FILL_BUFFER = fill_buffer/32bit/gpu.fill_buffer
FLATLAND = flatland/32bit/flatland
LIB_SIMPLE_FRAMEWORK3 = flatland/32bit/libsimple_framework3.so
...
define SLT_GPU_INSTALL_TARGET_CMDS
    ...
    $(INSTALL) -D -m 0644 $(@D)/$(LIB_SIMPLE_FRAMEWORK3) $(TARGET_DIR)/usr/lib/
    mkdir -p $(TARGET_DIR)/usr/share/arm
    cp -R $(@D)/flatland/samples/opengles_31  $(TARGET_DIR)/usr/share/arm
    ln  -s -f /usr/share/arm/opengles_31/compute_particles/assets  $(TARGET_DIR)/assets
endef
