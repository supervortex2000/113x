...
define UVC_GADGET_INSTALL_TARGET_CMDS
		...
		if [ "${BR2_PACKAGE_UVC_GADGET_USE_NN}" == "y" ]; then \
		  ...
		  $(INSTALL) -D -m 755 $(@D)/isp_sensor_detect.sh $(TARGET_DIR)/etc/ipc/; \
		  cp -af $(@D)/resolution $(TARGET_DIR)/etc/ipc/; \
        fi
endef
