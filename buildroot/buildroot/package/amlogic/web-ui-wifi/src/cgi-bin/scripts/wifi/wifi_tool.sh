...
ping_test()
{
	...
	ping $1 -w 5
	if [ $? -eq 1 ]; then
		echo "now going to ping router's ip: 114.114.114.114 for 5 seconds"
		ping 114.114.114.114 -w 5
	fi
	if [ $? -eq 1 ]; then
		echo "now going to ping router's ip: 8.8.8.8 for 5 seconds"
		ping 8.8.8.8 -w 5
	fi
	if [ $? -eq 1 ];then
		lightTest --num=12 --times=0 --speed=300 --time=0 --style=0 --mute_led=1 --listen_led=1
...
cat_wifi_node()
{
	file_node="/sys/bus/mmc/devices/mmc0:0000/mmc0:0000:1/device"
	if [ -f ${file_node} ]; then
		wifi_device_id=`cat ${file_node}`
		if [ ${wifi_device_id} = "0x8888" ];then
			AML_WIFI_FLAG="TRUE"
		fi
		return
	fi
	# av400 w1 wifi module
	file_node="/sys/bus/mmc/devices/mmc1:0000/mmc1:0000:1/vendor"
	if [ -f ${file_node} ]; then
		wifi_device_id=`cat ${file_node}`
		if [ ${wifi_device_id} = "0x8888" ];then
			AML_WIFI_FLAG="TRUE"
...
{
	cat_wifi_node
	if [ ${AML_WIFI_FLAG} = "TRUE" ];then
		aml_init
	else
		rtk_init
	fi
	parse_paras
	echo "wait for wpa_suppliant ready"
	while true; do
