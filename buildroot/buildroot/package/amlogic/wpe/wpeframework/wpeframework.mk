...
#WPEFRAMEWORK_VERSION = 53a4e35904641391dc5d04afa7b147c4fbd79706
#WPEFRAMEWORK_SITE = $(call github,rdkcentral,Thunder,$(WPEFRAMEWORK_VERSION))


WPEFRAMEWORK_VERSION = f53eec83fe4d09daff30f2bdbfa908e2d8dfa4f6
WPEFRAMEWORK_METHOD = git
WPEFRAMEWORK_SITE = git://github.com/rdkcentral/Thunder.git

WPEFRAMEWORK_INSTALL_STAGING = YES
#WPEFRAMEWORK_DEPENDENCIES = zlib $(call qstrip,$(BR2_PACKAGE_SDK_INSTALL)) host-wpeframework-tools rfc
...
#endif
WPEFRAMEWORK_CONF_OPTS += -DCDMI_ADAPTER_IMPLEMENTATION="amlogic-tvp"
endif
...
ifeq ($(BR2_PACKAGE_WPEFRAMEWORK_NETWORKCONTROL),y)
WPEFRAMEWORK_LAUNCH_SCRIPT = $(TARGET_DIR)/etc/init.d/S80WPEFramework
else
WPEFRAMEWORK_LAUNCH_SCRIPT = $(TARGET_DIR)/etc/init.d/S80WPEFramework
endif

define WPEFRAMEWORK_INSTALL_TARGET_CMDS
	install -d $(STAGING_DIR)/usr/include/WPEFramework/virtualinput/
	install -m 0644 $(@D)/Source/virtualinput/*.h  $(STAGING_DIR)/usr/include/WPEFramework/virtualinput/

	install -d $(STAGING_DIR)/usr/include/securityagent/
	#install -m 0755  $(@D)/Source/securityutility/lib/SecurityTokenUtil.h  $(STAGING_DIR)/usr/include/securityagent

##temp solution
	mkdir -p $(TARGET_DIR)/usr/lib/wpeframework/proxystubs
	install -d $(TARGET_DIR)/usr/bin
	install -d $(TARGET_DIR)/usr/lib
	cp -afv  $(STAGING_DIR)/usr/lib/libWPEFramework*.so*   $(TARGET_DIR)/usr/lib/
	cp -afv  $(STAGING_DIR)/usr/lib/wpeframework/proxystubs/libWPEFramework*.so*   $(TARGET_DIR)/usr/lib/wpeframework/proxystubs
	cp -afv  $(STAGING_DIR)/usr/lib/libocdm*.so*   $(TARGET_DIR)/usr/lib/
	cp -afv  $(STAGING_DIR)/usr/lib/libcompositorclient*.so*   $(TARGET_DIR)/usr/lib/
	cp -afv  $(STAGING_DIR)/usr/bin/WPEFramework*   $(TARGET_DIR)/usr/bin/
	cp -afv  $(STAGING_DIR)/usr/bin/WPEProcess*   $(TARGET_DIR)/usr/bin/
	install -d $(TARGET_DIR)/etc/WPEFramework
	install -m 0644  $(STAGING_DIR)/etc/WPEFramework/config.json  $(TARGET_DIR)/etc/WPEFramework

## Install WPEFramework launch script
	mkdir -p $(TARGET_DIR)/etc/init.d
	$(INSTALL) -D -m 0755 $(WPEFRAMEWORK_PKGDIR)/S80WPEFramework $(WPEFRAMEWORK_LAUNCH_SCRIPT)
## Youtube cerification Test
	$(INSTALL) -D -m 0755 $(WPEFRAMEWORK_PKGDIR)/wpe $(TARGET_DIR)/usr/bin/
	# default WPEFramework MAX Osd resultion is 1080
if [ "$(BR2_PACKAGE_WPEFRAMEWORK_MAXOSD_RESOLUTION_2160P)" == "y" ] ; then \
	sed -i 's/WESTEROS_GL_GRAPHICS_MAX_SIZE=1920x1080/WESTEROS_GL_GRAPHICS_MAX_SIZE=3840x2160/' $(WPEFRAMEWORK_LAUNCH_SCRIPT); \
else if [ "$(BR2_PACKAGE_WPEFRAMEWORK_MAXOSD_RESOLUTION_720P)" == "y" ] ; then \
	sed -i "s/WESTEROS_GL_GRAPHICS_MAX_SIZE=1920x1080/WESTEROS_GL_GRAPHICS_MAX_SIZE=1280x720/" $(WPEFRAMEWORK_LAUNCH_SCRIPT); \
fi \
fi
endef
...
WPEFRAMEWORK_PKGDIR = "$(TOP_DIR)/package/amlogic/wpe/wpeframework"
