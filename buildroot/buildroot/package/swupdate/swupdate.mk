...
SWUPDATE_DEPENDENCIES = aml-bootloader-message aml-ubootenv
...
SWUPDATE_MAKE_ENV+=SIGALG_RSAPSS=y
...
ifeq ($(BR2_PACKAGE_SWUPDATE_DOWNLOAD),y)
define SWUPDATE_SET_DOWNLOAD_OTA
	$(call KCONFIG_ENABLE_OPT,CONFIG_DOWNLOAD,$(SWUPDATE_BUILD_CONFIG))
	$(call KCONFIG_ENABLE_OPT,CONFIG_CURL,$(SWUPDATE_BUILD_CONFIG))
	$(call KCONFIG_ENABLE_OPT,CONFIG_CHANNEL_CURL,$(SWUPDATE_BUILD_CONFIG))
endef
endif
...
define SWUPDATE_SET_RECOVERY
endef
else
define SWUPDATE_SET_RECOVERY
	$(call KCONFIG_SET_OPT,CONFIG_EXTRA_CFLAGS,"-DHAVE_RECOVERY", \
		$(SWUPDATE_BUILD_CONFIG))
endef
endif
...
define SWUPDATE_KCONFIG_FIXUP_CMDS
	...
	$(SWUPDATE_SET_RECOVERY)
	$(SWUPDATE_SET_DOWNLOAD_OTA)
endef
...
define SWUPDATE_INSTALL_STAGING_CMDS
	$(INSTALL) -D -m 0644 $(@D)/include/progress_ipc.h $(STAGING_DIR)/usr/include/
	$(INSTALL) -D -m 0644 $(@D)/include/swupdate_status.h $(STAGING_DIR)/usr/include/
	$(INSTALL) -D -m 0644 $(@D)/libswupdate.so.$(LIBIPC_VERSION) $(STAGING_DIR)/usr/lib/
	ln -sf $(STAGING_DIR)/usr/lib/libswupdate.so.$(LIBIPC_VERSION) $(STAGING_DIR)/usr/lib/libswupdate.so
endef

define SWUPDATE_INSTALL_TARGET_CMDS
	$(INSTALL) -D -m 0755 $(@D)/swupdate $(TARGET_DIR)/usr/bin/swupdate
	$(INSTALL) -D -m 0755 $(@D)/libswupdate.so.$(LIBIPC_VERSION) $(TARGET_DIR)/usr/lib/
