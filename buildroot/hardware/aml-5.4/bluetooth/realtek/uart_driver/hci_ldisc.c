 ...
 * 
 * 	if (IS_ERR(skb)) {
 * 		RTKBT_ERR("command 0xfc19 tx failed (%ld)\n", PTR_ERR(skb));
 * 		return;
 * 	}
 * 
 * 	if (skb->len == 1)
 * 		RTKBT_INFO("hci flush sync status %u", skb->data[0]);
 * 
 * 	kfree_skb(skb);
 * #endif
 * }
...
static int __hci_uart_flush(struct hci_dev *hdev, u8 sync)
{
	...
	RTKBT_INFO("%s: hdev %p tty %p", __func__, hdev, tty);
...
{
	RTKBT_INFO("%s: hdev %p", __func__, hdev);
...
#endif
{
	...
	if (!hdev) {
		RTKBT_ERR("Frame for unknown device (hdev=NULL)");
		return -ENODEV;
	}
	...
	RTKBT_DBG("%s: type %d len %d", hdev->name, bt_cb(skb)->pkt_type,
	       skb->len);
			       ...
			       u32 plen, const void *param)
{
	...
	RTKBT_INFO("rtl: skb len %d", skb->len);
	 ...
	 * if (!test_bit(HCI_UART_PROTO_READY, &hu->flags)) {
	 * 	hci_proto_read_unlock(hu);
	 * 	RTKBT_ERR("rtl send: proto not ready");
	 * 	return -EUNATCH;
	 * }
				  ...
				  u16 *hci_rev, u16 *lmp_subver)
{
	...
	if (IS_ERR(skb)) {
		RTKBT_ERR("rtl: Could not read lmp subversion");
		return PTR_ERR(skb);
	}

	if (skb->len != sizeof(struct hci_rsp_read_local)) {
		RTKBT_ERR("%s: rtl: Local version length mismatch", hdev->name);
		kfree_skb(skb);
		return -EIO;
	}
...
static int rtkbt_lookup_le_device_poweron_whitelist(struct hci_uart *hu)
{
	...
	list_for_each_entry(p, &hu->hdev->le_conn_params, list) {
		...
		RTKBT_INFO("%s(): auto_connect = %d", __FUNCTION__, p->auto_connect);
		RTKBT_INFO("%s(): addr_type = 0x%02x", __FUNCTION__, p->addr_type);
		RTKBT_INFO("%s(): addr=%02x:%02x:%02x:%02x:%02x:%02x", __FUNCTION__,
		...
		if ( p->auto_connect == HCI_AUTO_CONN_ALWAYS &&
			...
			RTKBT_INFO("%s(): Set RTKBT LE Power-on Whitelist for "
			...
			if (!params) {
				RTKBT_ERR("Can't allocate memory for params");
				return -ENOMEM;
			}
			...
			if (result)
				RTKBT_ERR("rtl: Command failed for power-on whitelist");
...
static int rtkbt_set_le_device_poweron_data_filter(struct hci_uart *hu)
{
	...
	if (result)
		RTKBT_ERR("rtl: Command failed for set data filter");
...
static int rtkbt_simulate_disconnect_event(struct hci_uart *hu)
{
	...
	if (conn && (conn->state == BT_CONNECTED)){
		...
		RTKBT_INFO("Send Disconnect Complete EVENT to upper stack");
...
static int rtkbt_notify_suspend(struct hci_uart *hu)
{
	...
	if (result)
		RTKBT_ERR("Realtek suspend h5-bt failed");
...
static int hci_uart_pm_notifier(struct notifier_block *b, unsigned long v, void *d)
{
	...
	RTKBT_INFO("%s: %lu", __func__, v);
	switch (v) {
	case PM_SUSPEND_PREPARE:
		RTKBT_INFO("rtl: bt suspending");
#if WOBT_NOTIFY_BG_SCAN_LE_WHITELIST_ONLY
		...
		if (result)
			RTKBT_ERR("Realtek bg-scan h5-bt failed");
		...
		if (result < 0) {
			RTKBT_ERR("rtkbt_set_le_device_poweron_data_filter error: %d", result);
		}
#endif
		...
		if (result < 0) {
			RTKBT_ERR("rtkbt_notify_suspend error: %d", result);
		}
	...
	case PM_POST_SUSPEND:
		...
		if (result)
			break;
		RTKBT_INFO("rtl resume: hci ver %u, hci rev %04x, lmp subver %04x",
		...
		if (result < 0)
			RTKBT_ERR("rtkbt_simulate_disconnect_event error: %d", result);
		if (le_aoto_conn_always_exist(hu))
			le_scan_restart(hu);

		break;
	default:
		RTKBT_INFO("Caught msg %lu other than SUSPEND_PREPARE", v);
		break;
	}
...
static int hci_uart_tty_open(struct tty_struct *tty)
{
	...
	RTKBT_INFO("%s, tty %p", __func__, tty);
	...
	if (!(hu = kzalloc(sizeof(struct hci_uart), GFP_KERNEL))) {
		RTKBT_ERR("Can't allocate control structure");
		return -ENFILE;
	}
	...
	RTKBT_INFO("%s, register power off", __func__);
...
static int hci_uart_register_dev(struct hci_uart *hu)
{
	...
	RTKBT_INFO("hci_uart_register_dev");

	/* Initialize and register HCI device */
	hdev = hci_alloc_dev();
	if (!hdev) {
		RTKBT_ERR("Can't allocate HCI device");
		return -ENOMEM;
	}
	...
	if (hci_register_dev(hdev) < 0) {
		RTKBT_ERR("Can't register HCI device");
		hci_free_dev(hdev);
		return -ENODEV;
	}
...
static int __init hci_uart_init(void)
{
	...
	RTKBT_INFO("HCI UART driver ver %s", VERSION);
...
#endif
		RTKBT_ERR("HCI line discipline registration failed. (%d)", err);
		return err;
	}
...
static void __exit hci_uart_exit(void)
{
	...
	if ((err = tty_unregister_ldisc(N_HCI)))
		RTKBT_ERR("Can't unregister HCI line discipline (%d)", err);
#endif
