				      ...
				      int len, int pkt_type)
{
	...
	RTKBT_DBG("We request packet no %u to card", tmp);
	...
	if (rel) {
		...
		spin_unlock_irqrestore(&h5->lock, flags);
		RTKBT_DBG("Sending packet with seqno %u", tmp);
	}
...
static struct sk_buff *h5_dequeue(struct hci_uart *hu)
{
	...
	if ((skb = skb_dequeue(&h5->unrel)) != NULL) {
		...
		if (nskb) {
			...
			RTKBT_ERR("Could not dequeue pkt because alloc_skb failed");
		}
	}
...
static void h5_pkt_cull(struct h5_struct *h5)
{
	...
	if (h5->rxack != seqno)
		RTKBT_ERR("Peer acked invalid packet");

	RTKBT_DBG("Removing %u pkts out of %u, up to seqno %u",
	       pkts_to_be_removed, skb_queue_len(&h5->unack),
	       (seqno - 1) & 0x07);
	...
	if (i != pkts_to_be_removed)
		RTKBT_ERR("Removed only %u out of %u pkts", i, pkts_to_be_removed);
}
...
static void h5_handle_le_pkt(struct hci_uart *hu)
{
	...
	/* spot "conf resp" pkts */
		...
		RTKBT_DBG("Found a LE conf resp pkt, device go into active state");
	...
	/* Spot "sync" pkts. If we find one...disaster! */
		...
		RTKBT_ERR("Found a LE sync pkt, card has reset");
		//DO Something here
	}
	...
	/* Spot "wakeup" pkts. reply woken message when in active mode */
		...
		RTKBT_ERR("Found a LE Wakeup pkt, and reply woken message");
		//      DO Something here
	...
	/* Spot "woken" pkts. receive woken message from device */
		...
		RTKBT_ERR("Found a LE woken pkt from device");
		//      DO Something here
	}
	/* Spot "Sleep" pkts */
	else if (h5->rx_skb->data[1] >> 4 == 2 && h5->rx_skb->data[2] == 0 &&
...
skb->data[4], sleep_pkt, 2)) {
		RTKBT_ERR("Found a LE Sleep pkt");
		//      DO Something here
	}
}
#endif
...
static inline void h5_unslip_one_byte(struct h5_struct *h5, unsigned char byte)
{
	...
	case H5_ESCSTATE_ESC:
		...
		default:
			RTKBT_ERR("Invalid byte %02x after esc byte", byte);
			kfree_skb(h5->rx_skb);
			h5->rx_skb = NULL;
...
static void h5_complete_rx_pkt(struct hci_uart *hu)
{
	...
	if (h5->rx_skb->data[0] & 0x80) {	/* reliable pkt */
		...
		RTKBT_DBG("Received seqno %u from card", rxseq);
	}

	h5->rxack = (h5->rx_skb->data[0] >> 3) & 0x07;
	RTKBT_DBG("Request for pkt %u from card", h5->rxack);
	...
	if (!pass_up) {
		...
		if (desc != H5_ACK_PKT && desc != H5_LE_PKT) {
			 ...
			 * } else { */
				RTKBT_ERR("Packet for unknown channel (%u %s)",
...
static int h5_recv(struct hci_uart *hu, void *data, int count)
{
	...
	RTKBT_DBG("hu %p count %d rx_state %d rx_count %ld",
	...
	while (count) {
		...
		case H5_W4_HDR:
			if ((0xff & (u8) ~
				...
				RTKBT_ERR("Error in H5 hdr checksum");
				kfree_skb(h5->rx_skb);
				h5->rx_state = H5_W4_PKT_DELIMITER;
				h5->rx_count = 0;
				continue;
			...
			if (h5->rx_skb->data[0] & 0x80	/* reliable pkt */
				...
				RTKBT_ERR("Out-of-order packet arrived, got %u expected %u",
				       h5->rx_skb->data[0] & 0x07, rxseq);
		...
		case H5_W4_PKT_DELIMITER:
			...
			case 0xc0:
				...
				break;
			default:
				/*RTKBT_ERR("Ignoring byte %02x", *ptr); */
				break;
			}
			ptr++;
			count--;
		...
		case H5_W4_PKT_START:
			...
			default:
				...
				if (!h5->rx_skb) {
					RTKBT_ERR("Can't allocate mem for new packet");
					h5->rx_state = H5_W4_PKT_DELIMITER;
					h5->rx_count = 0;
					return 0;
				}
...
static void h5_timed_event(struct work_struct *work)
{
	...
	RTKBT_INFO("hu %p retransmitting %u pkts", hu, h5->unack.qlen);
...
int h5_init(void)
{
	...
	if (!err)
		RTKBT_INFO("HCI Realtek H5 protocol initialized");
	else
		RTKBT_ERR("HCI Realtek H5 protocol registration failed");
