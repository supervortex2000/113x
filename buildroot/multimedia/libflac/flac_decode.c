...
typedef struct FLACContext {
    ...
    unsigned int error_num;                 ///< error decoded num(for get error num)
} FLACContext;
...
static void allocate_buffers(FLACContext *s)
{
    ...
    for (i = 0; i < s->channels; i++) {
        ...
        if (s->decoded[i] == NULL) {
            audio_codec_print("av_realloc decoded[%d] NULL, %s,%d", i, __func__, __LINE__);
        }
    }
}
                              ...
                              const uint8_t *buffer)
{
    ...
    audio_codec_print("## METADATA sr=%d, ch=%d, bps=%d,-------------\n",
...
static int decode_frame(FLACContext *s)
{
    ...
    if (fi.channels != s->channels) {
        audio_codec_print("switching channel layout[%d->%d] mid-stream is not supported\n", s->channels, fi.channels);
        return -1;
    }
    s->ch_mode = fi.ch_mode;

    if (fi.bps && fi.bps != s->bps) {
        audio_codec_print("switching bps[%d->%d] mid-stream is not supported\n", s->bps, fi.bps);
        return -1;
    }
    if (0/*s->bps > 16*/) {
...
int audio_dec_decode(audio_decoder_operations_t *adec_ops, char *outbuf, int *outlen, char *inbuf, int inlen)
{
    ...
    int max_framesize = FFMAX(17 * s->max_framesize / 16 + 32, s->max_framesize);
...
int audio_dec_init(audio_decoder_operations_t *adec_ops)
{
    ...
    FLACContext *s = &flactext;
    s->avctx = &acodec;
    audio_codec_print("[%s]BuildDate--%s  BuildTime--%s\n", __FUNCTION__, __DATE__, __TIME__);
    avctx->sample_fmt = SAMPLE_FMT_S16;
    avctx->extradata = adec_ops->extradata;
    avctx->extradata_size = adec_ops->extradata_size;

    if (!avctx->extradata_size) {
        audio_codec_print("%s[%d]extradata_size is %d\n", __func__, __LINE__, avctx->extradata_size);
        return 0;
    }
...
int audio_dec_release(audio_decoder_operations_t *adec_ops)
{
    ...
    for (i = 0; i < flactext.channels; i++) {
        if (flactext.decoded[i] != NULL) {
            //av_freep(&flactext.decoded[i]);
            free(flactext.decoded[i]);
            flactext.decoded[i] = NULL;
        }
    }
...
{
    if (pAudioInfo != NULL) {
        ((AudioInfo *)pAudioInfo)->channels = flactext.channels;
        ((AudioInfo *)pAudioInfo)->samplerate = flactext.samplerate;
        ((AudioInfo *)pAudioInfo)->error_num = flactext.error_num;
        ((AudioInfo *)pAudioInfo)->drop_num = flactext.error_num;
        #if 0
        audio_codec_print("%s:%d ch(%d) sr(%d)  stream_error_num(%u) ",
        __FUNCTION__, __LINE__, flactext.channels , flactext.samplerate, flactext.error_num );
        #endif
    }
    return 0;
}
